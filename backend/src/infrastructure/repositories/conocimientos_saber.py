"""SQLAlchemy repository for SABER knowledge persistence."""
...
class ConocimientoSaberRepository:
    """Read and write knowledge items constrained to type SABER."""
    ...
    async def list_by_competencia(
        ...
        """Return SABER knowledge items linked to the given competence."""
        statement = (
            select(Conocimiento)
            .where(Conocimiento.competencia_id == competencia_id)
            .where(Conocimiento.tipo == TipoConocimiento.SABER)
    ...
    async def get_by_id_for_competencia(
        ...
        """Return a SABER knowledge item only when it belongs to the competence."""
        statement = (
            select(Conocimiento)
    ...
    async def descripcion_exists(
        ...
        """Check duplicate SABER description inside a competence."""
        statement = (
    ...
    async def add_conocimiento(
        ...
        """Create a SABER knowledge item for one competence."""
    ...
    async def save_conocimiento(self, conocimiento: Conocimiento) -> Conocimiento:
        """Flush pending changes to a SABER knowledge item."""
        self._session.add(conocimiento)
        await self._session.flush()
        return conocimiento

    async def delete_conocimiento(self, conocimiento: Conocimiento) -> None:
        """Delete a SABER knowledge row."""
