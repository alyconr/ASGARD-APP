...
async def download_program_pdf(
    ...
    doc_info = payload.get("documental", {}).get("programa_pdf", {}).get("documento", {})
    storage_key = doc_info.get("storage_key")
    filename = doc_info.get("original_filename") or "programa-formacion.pdf"

    if not storage_key:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="El programa no tiene un archivo PDF de evidencia cargado",
        )
