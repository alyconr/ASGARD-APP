"""Application service for TASK-11 PROCESO knowledge CRUD."""
...
class ConocimientoProcesoError(Exception):
    """Base exception for PROCESO knowledge use-case errors."""


class ConocimientoProcesoDraftNotFoundError(ConocimientoProcesoError):
    """Raised when no program draft exists for a reference."""


class ConocimientoProcesoCompetenciaNotFoundError(ConocimientoProcesoError):
    """Raised when the competence is missing or not tied to the program."""


class ConocimientoProcesoValidationError(ConocimientoProcesoError):
    """Raised when user input is not valid."""


class ConocimientoProcesoDuplicateError(ConocimientoProcesoError):
    """Raised when a PROCESO description already exists in the competence."""


class ConocimientoProcesoNotFoundError(ConocimientoProcesoError):
    """Raised when a PROCESO knowledge item is missing."""


class ConocimientoProcesoRepositoryProtocol(Protocol):
    """Repository behavior required by the PROCESO knowledge service."""
    ...
    async def list_by_competencia(
        ...
        """List competence PROCESO knowledge items."""
    ...
    async def get_by_id_for_competencia(
        ...
        """Return a PROCESO knowledge item for a competence."""
    ...
    async def descripcion_exists(
        ...
        """Return whether a PROCESO description exists in a competence."""
...
class ProgramaConocimientoProcesoService:
    """Coordinate PROCESO knowledge CRUD within the active program draft."""

    def __init__(
        self,
        session: AsyncSessionProtocol,
        conocimiento_repository: ConocimientoProcesoRepositoryProtocol,
    ...
    async def list_conocimientos(
        ...
        """List PROCESO knowledge items tied to the current competence."""
    ...
    async def create_conocimiento(
        ...
        """Create a PROCESO knowledge item for the current competence."""
        ...
        await self._audit_repository.add_event(
            ...
            accion="CONOCIMIENTO_PROCESO_CREADO",
            detalle={
                "referencia_id": str(referencia_id),
                "competencia_id": str(competencia.id),
                "tipo": TipoConocimiento.PROCESO.value,
            },
        )
        await self._session.commit()
    ...
    async def update_conocimiento(
        ...
        """Update a PROCESO knowledge item without changing its competence."""
        command = _normalize_payload(payload)
        draft = await self._get_program_draft(referencia_id)
        competencia = await self._require_competencia(draft, competencia_id)
        ...
        conocimiento.tipo = TipoConocimiento.PROCESO
        ...
        await self._audit_repository.add_event(
            entidad="Conocimiento",
            entidad_id=conocimiento.id,
            accion="CONOCIMIENTO_PROCESO_ACTUALIZADO",
            detalle={
                "referencia_id": str(referencia_id),
                "competencia_id": str(competencia.id),
                "tipo": TipoConocimiento.PROCESO.value,
    ...
    async def delete_conocimiento(
        ...
        await self._audit_repository.add_event(
            ...
            accion="CONOCIMIENTO_PROCESO_ELIMINADO",
            detalle={
                "referencia_id": str(referencia_id),
                "competencia_id": str(competencia.id),
                "tipo": TipoConocimiento.PROCESO.value,
        ...
        return ConocimientoProcesoDeleteDTO(
            referencia_id=referencia_id,
            competencia_id=competencia.id,
    ...
    async def _get_program_draft(self, referencia_id: uuid.UUID) -> BorradorSesion:
        ...
        if draft is None:
            raise ConocimientoProcesoDraftNotFoundError(
    ...
    async def _sync_draft_curricular_payload(
        ...
        draft: BorradorSesion,
        competencia_id: uuid.UUID,
        conocimientos_proceso: list[Conocimiento],
    ) -> None:
        ...
        if comp_idx >= 0:
            ...
            if isinstance(competencia_data, dict):
                ...
                preserved = [
                    ...
                    and item.get("tipo") != TipoConocimiento.PROCESO.value
                ]
                competencia_data["conocimientos"] = preserved + [
                    _conocimiento_payload_item(item) for item in conocimientos_proceso
                ]
...
def _normalize_payload(
    ...
    if not descripcion:
        raise ConocimientoProcesoValidationError("La descripcion es obligatoria")
    return ConocimientoProcesoPayloadDTO(descripcion=descripcion)
...
def _build_conocimiento_dto(conocimiento: Conocimiento) -> ConocimientoProcesoDTO:
    return ConocimientoProcesoDTO(
        id=conocimiento.id,
